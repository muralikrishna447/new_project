...
describe Shopify::Order, :skip => 'true' do
  ...
  JOULE_ORDER_WITHOUT_CUSTOMER = 4507802
  ...
  context 'order fulfillment' do
    ...
    context 'orders with customers' do
      before(:each) do
    ...
    it 'fulfills a simple joule order without a customer' do
      # Not stubbing fulfillment call since this is not made for joule
      WebMock::stub_request(:put, /\.com\/admin\/orders\/4507800.json/).
        to_return(:status => 200, :body => "", :headers => {})
      stub_metafield_get
      ...
      @user.premium?.should == false
      @user.joule_purchase_count.should == 0
      order = Shopify::Order.find(JOULE_ORDER_WITHOUT_CUSTOMER)
      order.process!
      @user.reload
      @user.joule_purchase_count.should == 0
      # No premium with joule!
