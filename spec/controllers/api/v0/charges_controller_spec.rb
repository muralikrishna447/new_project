  ...
  context 'POST /create' do
    ...
    context 'logged in' do
      ...
      it 'should queue charge when called correctly' do
        ...
        post :create, sku: "cs10002", stripeToken: 'xxx', price: '50.0'
      ...
      it "should create the stripe_order object and set the user to premium when ordering the circulator" do
        options = {sku: 'cs10001', stripeToken: 'xxx', price: '200.0', gift: false}.merge(@billing_address).merge(@shipping_address)
        post :create, options
      ...
      it "should create the stripe_order object and set the user to premium when ordering the circulator" do
        ...
        options = {sku: 'cs10001', stripeToken: 'xxx', price: '170.0', gift: false}.merge(@billing_address).merge(@shipping_address)
        post :create, options
        expect(response.status).to eq(200)
        stripe_order = StripeOrder.last
      ...
      it "should create the stripe_order without the billing and shipping" do
        options = {sku: 'cs10002', stripeToken: 'xxx', price: '50.0', gift: false}
        post :create, options
        expect(response.status).to eq(200)
        stripe_order = StripeOrder.last
        stripe_order.user_id.should == @user.id
      ...
      it 'should let you buy a premium class if you already have one if it is a gift' do
        @user.make_premium_member(10)
        post :create, sku: 'cs10002', stripeToken: 'xxx', price: '50.0', gift: true
        expect(response.status).to eq(200)
      end
