...
describe ActsAsRevisionable::RevisionRecord do
  ...
  it "should be able to restore the original model using Ruby serialization" do
    ...
    restored.attributes.except('id').should == attributes.except('id')
  end
  ...
  it "should be able to restore associations" do
    ...
    revision.data = Zlib::Deflate.deflate(Marshal.dump(attributes))
    TestRevisionableRecord.stub(:new).and_return(restored)
    revision.should_receive(:restore_association).with(restored, :associations, {'id' => 2, 'value' => 'val'}, false)
    restored = revision.restore
  end
  ...
  it "should be able to restore a record for a model that has changed and add errors to the restored record" do
    ...
    restored.stub(:associations).and_return(associations)
    associated_record = TestRevisionableAssociationRecord.new
    associations.stub(:build).and_return(associated_record)
  ...
  it "should be able to truncate the revisions for a record" do
    ...
    ActsAsRevisionable::RevisionRecord.any_instance.stub(:where).with(['revisionable_type = ? AND revisionable_id = ?', 'TestRevisionableRecord', 1], :offset => 15, :order => 'revision DESC', :limit => 1).and_return(revision)
    ActsAsRevisionable::RevisionRecord.any_instance.stub(:delete_all).with(['revisionable_type = ? AND revisionable_id = ? AND revision <= ?', 'TestRevisionableRecord', 1, 20])
  ...
  it "should be able to truncate the revisions for a record by age" do
    ...
    revision.revision = 20
    time = 2.weeks.ago
    minimum_age = double('minimum_age')
    allow(minimum_age).to receive_message_chain(:seconds, :ago => time)
    ActsAsRevisionable::RevisionRecord.stub_chain(:where, :order, :offset, :first).and_return(revision)
    ActsAsRevisionable::RevisionRecord.stub_chain(:where, :delete_all)
  ...
  it "should not truncate the revisions for a record if it doesn't have enough" do
    ActsAsRevisionable::RevisionRecord.stub_chain(:where, :order, :offset, :first).and_return(nil)
  ...
  it "should be able to find a record by revisioned type and id" do
    revision = ActsAsRevisionable::RevisionRecord.new(TestRevisionableRecord.new(:name => 'name'))
    ActsAsRevisionable::RevisionRecord.stub_chain(:where, :first).and_return(revision)
  ...
  it "should find the last revision" do
    revision = ActsAsRevisionable::RevisionRecord.new(TestRevisionableRecord.new(:name => 'name'))
    ActsAsRevisionable::RevisionRecord.stub_chain(:where, :order, :first).and_return(revision)
