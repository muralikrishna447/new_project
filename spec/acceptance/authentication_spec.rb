...
feature 'user authentication', :js do
  ...
  scenario 'user is prompted with a signup popup after viewing 3 activities', js_errors: false do
    i = 0
    4.times do
      i+=1
      instance_variable_set("@activity_#{i}", Fabricate(:activity, title: "test_#{i}", published: true))
    end
    # visit activity_url(@activity_1)
    # visit activity_url(@activity_2)
    # visit activity_url(@activity_3)
    # visit activity_url(@activity_4)
