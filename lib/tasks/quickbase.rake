namespace :quickbase do
  task :generate_report => :environment do
    started_at = Time.now
    Rails.logger.info "Quickbase:GenerateReport - Starting #{Time.now}"
    sqs = Aws::SQS::Client.new(region: 'us-east-1')
    serial_numbers = []
    circulator_addresses_to_gather = []
    Rails.logger.info "Quickbase:GenerateReport - Retrieving records"
    ...
    Rails.logger.info "Quickbase:GenerateReport - Got #{results["serial_number"].length} records back"
    ...
    0.upto(count) do |x|
      ...
      Rails.logger.info "Quickbase:GenerateReport - On #{record_id} (Serial: #{serial_number}) from #{created_at}"
      if serial_number.nil?
        Rails.logger.info "Quickbase:GenerateReport - skipping record #{x}"
        next
      end
      if (started_at - created_at) > 3.days
        ...
        Rails.logger.info "Quickbase:GenerateReport - Updating #{record_id} (Serial: #{serial_number}) with \n#{circulator_addresses.join("\n")}"
        ...
        Rails.logger.info "Quickbase:GenerateReport - Marked #{record_id} as records_requested"
      end
    end
    ...
    circulator_addresses_to_gather.in_groups_of(512, false) do |circ_add|
      body = {
        "jobName" => "#{start_time}_#{end_time}_#{counter}",
        "dateRange" => {
          "start" => start_time,
          "end" => end_time
        },
      ...
      Rails.logger.info "Quickbase:GenerateReport - Sending \n" + body.to_json
      ...
      Rails.logger.info "Quickbase:GenerateReport - Got response #{resp.inspect}"
      counter+=1
    end
    Rails.logger.info "Quickbase:GenerateReport - Sent #{serial_numbers.length} records"
    finished_at = Time.now
    run_time = finished_at-started_at
    Rails.logger.info "Quickbase:GenerateReport - Ran for #{run_time.to_i} secconds"
  end
end
