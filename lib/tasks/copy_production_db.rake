...
task :copy_production_db do
  ...
  sh %{heroku pgbackups:capture --expire --remote production --app chefsteps-production}
  sh %{curl -o /tmp/latest.dump `heroku pgbackups:url --remote production --app chefsteps-production`}
