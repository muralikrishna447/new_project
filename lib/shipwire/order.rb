  ...
  class Order
    ...
    def joule_line_item(shopify_order)
      ...
      shopify_order.line_items.each do |line_item|
        joule_line_items << line_item if line_item.sku == Shopify::Order::JOULE_SKU
      end
      if joule_line_items.length > 1
        raise "Order with id #{shopify_order.id} has multiple Joule line items with sku #{Shopify::Order::JOULE_SKU}, expected only one"
