class Activity < ActiveRecord::Base
  ...
  # The explicit steps_activities bit here is to make it compatible with the alias generated by pg_search_scope
  # The logic here has to match that in $scope.activityImageFpfile !!
  # scope :has_featurable_image, joins(:steps, 'JOIN "steps" "steps_activities" ON "steps_activities"."activity_id" = "activities"."id"').where(Activity.arel_table[:image_id].not_eq('').or(Activity.arel_table[:featured_image_id].not_eq('')).or(Step.arel_table.alias("steps_activities")[:image_id].not_eq('')))
  # ... well, that was a clever idea but dead slow.
  ...
  attr_accessible :source_activity, :source_activity_id, :source_type, :author_notes, :currently_editing_user, :include_in_gallery
