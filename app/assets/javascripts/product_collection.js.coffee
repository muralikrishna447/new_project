...
$ ->
  ...
  $(window).scroll ->
    if $(window).scrollTop() <= 60
      if window_width > 767
        $('#product-intro').fadeIn 'fast', 'easeInOutQuad'
      $('.product').removeClass 'alt'
    else
      if window_width > 767 
        $('#product-intro').fadeOut 'fast','easeInOutQuad' 
      $('.product').addClass 'alt'
...
window.finishproductSplit = ->
  $.post '/splitty/finished?experiment=product_collection'
