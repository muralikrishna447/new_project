class FeaturedController < ApplicationController
  def index
    ...
    recipes_hash = recipes.map{|r| {title: r.title, description: r.description, image: filepicker_arbitrary_image(r.featured_image,1600), url: activity_url(r)}}
    classes = Assembly.pubbed_courses.order('created_at desc').limit(3).to_a
    classes_hash = classes.map{|c| {title: c.title, description: c.description, image: filepicker_arbitrary_image(c.image_id,1600), url: landing_class_path(c)}}
