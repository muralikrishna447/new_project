    ...
    class RandomDropsController < BaseController
      ...
      def show
        if (params[:id] == @user_id_from_token.to_s) || current_user.role == 'admin'
          ...
          @random_drop['redirect'] = "https://store.#{Rails.application.config.shared_config[:chefsteps_endpoint]}/cart/#{@random_drop['variant_id']}:1?discount=#{@random_drop['discount_code']}"
